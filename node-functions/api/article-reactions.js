...
async function toggleReaction(articleId, username, action, add){
  ...
  const countKey = action === 'like' ? userLikeCountKey(username) : userFavCountKey(username);
  if(add){
    const alreadyLiked = username in data;
    data[username] = nowIso();
    if(!alreadyLiked){
      await redis.incr(countKey); // 计数器 +1
    }
  }else{
    const wasLiked = username in data;
    delete data[username];
    if(wasLiked){
      await redis.decr(countKey); // 计数器 -1
    }
  }
...
export async function onRequestGet(context){
  try{
    ...
    // 模式2: 获取某用户的点赞/收藏总数（优先读计数器 O(1)，回退到扫描）
    if(username){
      ...
      if(likeCounter !== null && favCounter !== null){
        return json({ ok: true, likeCount: parseInt(likeCounter) || 0, favoriteCount: parseInt(favCounter) || 0 });
      }
      // 回退：扫描全量 hash（兼容旧数据）
      const [likesRaw, favsRaw] = await Promise.all([
      ...
      try{
        ...
        if(favCount > 0) await redis.set(userFavCountKey(username), favCount);
      }catch(_){}

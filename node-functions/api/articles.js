...
function sanitizeArticle(input = {}, fallbackAuthor = ''){
  ...
  return {
    ...
    updatedAt: now,
    // 保留已有的点赞/收藏数据（如果文章记录中已存在）
    likes: (input.likes && typeof input.likes === 'object' && Object.keys(input.likes).length) ? input.likes : {},
    favorites: (input.favorites && typeof input.favorites === 'object' && Object.keys(input.favorites).length) ? input.favorites : {}
  };
}
...
function toArticleMeta(article){
  return {
    ...
    // 点赞/收藏计数（从内嵌数据读取）
    likeCount: article.likes && typeof article.likes === 'object' ? Object.keys(article.likes).length : 0,
    favoriteCount: article.favorites && typeof article.favorites === 'object' ? Object.keys(article.favorites).length : 0
  };
}
      ...
      try{
        ...
        for(const raw of (rawArticles || [])){
          try{
            ...
            if(username in favs) favoriteCount++;
          }catch(_){}
        }
      }catch(_){}
    }

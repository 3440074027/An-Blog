 ...
 * - user / friends / chat 是用户级：用 Authorization 或 ?username= 解析当前账号。
 *
...
export async function onRequestGet(context){
  try{
    ...
    if(username){
      ...
      fields.push(VERSION_FIELDS.friends(username));
      fields.push(VERSION_FIELDS.chat(username));
    }
    ...
    if(username){
      ...
      result.friends = values[3] || 0;
      result.chat = values[4] || 0;
